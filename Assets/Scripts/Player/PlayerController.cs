...
public class PlayerController : MonoBehaviour
{
    ...
    private float dashColdTime = 5.0f;
    ...
    private float skillColdTime = 30.0f;
        ...
        if (PV.IsMine)
        {
            ...
            if (_bIsDash == true)
            {
                ...
                else
                {
                    ...
                    dahsColdBtn.GetComponent<Image>().fillAmount = dashCold/dashColdTime;
            ...
            if (_bIsSkill == true)
            {
                if (_bIntoCold) //Into cold time
                {
                    ...
                    skillColdBtn.GetComponent<Image>().fillAmount = skillCold / skillColdTime;
                    if (skillCold >= skillColdTime)
                    {
                        skillTime = 0.0f;
                        _bIsSkill = false;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if ((int)PhotonNetwork.CurrentRoom.CustomProperties["PlayerOnDuoTreasure"] == 2)
        {
            ...
            GameObject.Find("TwoPersonTreasure").transform.GetChild(1).gameObject.SetActive(true);
